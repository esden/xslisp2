...
#define BUFSIZE_SMALL 64  // size for small buffers
...
unsigned char rcx_rdbuf[BUFSIZE];
...
int rdbuf_ready = 0;
unsigned char com_dest;
unsigned char com_srcport = 1;

#define RCX_DATA_PORT 2
unsigned char rcx_rdbuf_ready = 0;
unsigned char rcx_data_mode = RCX_MODE_OVERWRITE;
...
void rcx_input_handler(const unsigned char *buf, unsigned char len, unsigned char src)
{
...
static int transmit1(int ntries) {
	...
	while (--ntries >= 0) {
		ack_flag = 0;
		lnp_addressing_write(wtbuf, len, com_dest, com_srcport);
		msleep(250+5*len);
		if (ack_flag)
			return 1;
	}
...
wakeup_t rcx_rdbuf_ready_p( wakeup_t data )
{
    return rcx_rdbuf_ready || interrupt_flag;
}

#define wait_rdbuf_ready() wait_event(rdbuf_ready_p, 0)
#define wait_rcx_rdbuf_ready() wait_event(rcx_rdbuf_ready_p, 0)
...
LOOP:
	...
	SUBR_CALL:
        ...
        case Lset_receive_mode:
            e = valINT( rcx_data_mode );
            rcx_data_mode = INTval( base[0] );
            break;
        case Lget_receive_mode:
            e = valINT( rcx_data_mode );
            break;
        case Lunlock_receive_buffer:
            ...
            break;
        case Lsender:
            e = rcx_rdbuf_ready ? valINT( rcx_rdbuf[0] ) : FALSE;
            break;
        case Lreceive:
            {
                wait_rcx_rdbuf_ready();
                if( (e = rcx_read()) == ERR ) goto LERROR;
                break;
            }
        case Linput:
...
void begin_rcx() {
	...
	lnp_addressing_set_handler(RCX_DATA_PORT, &rcx_input_handler);
#endif
