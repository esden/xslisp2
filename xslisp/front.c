...
void initGlobals() {
	...
	def("port-on", MKCONST(Lport_on,1,0,0));
	def("port-off", MKCONST(Lport_off,1,0,0));
    def("port", MKCONST(Lport,1,0,0));
    ...
    def("transmit", MKCONST(Ltransmit,2,0,0));
    def("set-receive-mode", MKCONST(Lset_receive_mode,1,0,0));
    def("get-receive-mode", MKCONST(Lget_receive_mode,0,0,0));
    def("unlock-receive-buffer", MKCONST(Lunlock_receive_buffer,0,0,0));
                    ...
                    unsigned char src)
{
	...
	if (rdbuf_ready) {
    	...
    	CMDbuf[0] = CMDACK;
    	lnp_addressing_write(CMDbuf, 1, src, 0);
    	for (i = 0; i < len; i++)
    		rdbuf[i] = buf[i];
    	rdbufp = rdbuf;
    	rdbuf_ready = 1;
...
void begin_rcx() {
	...
	CMDbuf[0] = CMDPING;
	for (i = 0; i < 5; i++) {
		lnp_addressing_write(CMDbuf, 1, hostaddr << 0x04 | 0x01, 0);
		for (j = 0; j < 10; j++) {
#ifndef _WIN32
			if (tty_usb)
#endif
...
static void transmit() {
	rdbuf_ready = 0;
	lnp_addressing_write(wtbuf, wtbufp-wtbuf, hostaddr << 0x04 | 0x01, 0);
}
	...
	initGlobals();
 	...
 	while(--argc > 0) {
 		if (strncmp("--tty=",argv[0],6) == 0) {
 	    	...
 	    	com_debug = 1;
 	    } else if (strncmp("--rcxaddr=", argv[0], 10) == 0) {
            hostaddr = atoi( &argv[0][10] );
        } else if (strcmp("--rcxaddr", argv[0]) == 0) {
            argv++;
            hostaddr = atoi( argv[0] );
        }
 	    argv++;
 	}

    printf( "Using hostaddr: %d\n", hostaddr );
	begin_rcx();
#ifdef ONLINE
	signal(SIGINT,sigint);
#endif
